  ...
  if (leftBracket > -1) {
    ...
    components.forEach((c) => {
      ...
      if (trimmed.startsWith(prefix)) {
        ...
        if (useSource) {
          styleImports.push(`import '${lib}/${formatMap[format]}/components/${hyphenate(component)}/style/index'`);
        } else {
...
var getLocaleRE = (options) => new RegExp(`${escapeStringRegexp(`${options.lib}/`)}(es|lib)${escapeStringRegexp("/hooks/use-locale/index")}`);
    ...
    vite: {
      config(config) {
        ...
        if (options.defaultLocale) {
          ...
          config.optimizeDeps.esbuildOptions.plugins.push(getViteDepPlugin(options));
        }
      }
    }
  };
